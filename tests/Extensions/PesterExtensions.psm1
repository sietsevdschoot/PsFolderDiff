...
Function BeJsonEquivalentTo {
    ...
    $expectedJson = ($Expected | ConvertTo-Json -Depth $Depth)
    $actualJson = ($ActualValue | ConvertTo-Json -Depth $Depth)

    $success = (-not $Negate) -eq ($actualJson -eq $expectedJson)

    if (!$success) {

        $failureMessage = $Negate `
            ? "Expected object not to be equivalent to:`n$($expectedJson)" `
            : "Expected:`n$($expectedJson)`nBut found: `n$($actualJson)"
    }
...
Export-ModuleMember -Function BeJsonEquivalentTo
