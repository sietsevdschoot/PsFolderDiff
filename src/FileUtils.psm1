function Copy-KeepExisting {
    ...
    PROCESS {
        ...
        $destinationParentFolders = ($destinationPath.FullName -replace [regex]::Escape($destinationPath.Root)) -split [regex]::Escape([IO.Path]::DirectorySeparatorChar)
        ...
        elseif ($firstMatchingExistingFolder) {
            ...
            $destinationFolder = Join-Path $destinationPath (($fileParentFolders | Select-Object -Skip ($fileParentFolders.IndexOf($firstMatchingExistingFolder))) -join [IO.Path]::DirectorySeparatorChar)
        }
        else {
